...
new strategies integration (MACD, Momentum, Mean Reversion, VWAP, Stochastic Oscillator),
commission handling, trade logging, configuration export, and risk parity position sizing.
"""
...
class MomentumStrategy(Strategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        signals = pd.DataFrame(index=data.index)
        signals['momentum'] = data['Close'] - data['Close'].shift(self.window)
        signals['signal'] = np.where(signals['momentum'] > self.threshold, 1.0, 0.0)
        signals['positions'] = pd.Series(signals['signal']).diff().fillna(0.0)
        return signals
...
class Backtester:
    def __init__(self, data: pd.DataFrame, signals: pd.DataFrame, initial_capital: float = 100000.0, shares: int = 100,
                 position_sizing: str = "fixed", risk_fraction: float = 0.01, commission: float = 0.001):
        self.data = data
        self.signals = signals
        self.initial_capital = initial_capital
        self.shares = shares
        self.position_sizing = position_sizing.lower()  # "fixed", "dynamic", "fixed fractional", "risk parity"
        self.risk_fraction = risk_fraction
        self.commission = commission  # commission as a percentage (e.g., 0.001 = 0.1%)
    ...
    def run_backtest(self) -> pd.DataFrame:
        ...
        for i in range(len(self.data)):
            ...
            if position == 0 and signal == 1:
                ...
                elif self.position_sizing == "risk parity":
                    volatility = self.data['Close'].pct_change().rolling(window=20, min_periods=1).std().values[i]
                    if volatility > 0:
                        shares_to_buy = int((cash * self.risk_fraction) / (volatility * price))
                    else:
                        shares_to_buy = self.shares
                else:
                    shares_to_buy = self.shares
                ...
                self.trades.append({"date": date, "action": "buy", "shares": shares_to_buy, "price": price, "commission": commission_cost})
            # Sell signal
            elif position > 0 and signal == 0:
                proceeds = position * price
                commission_cost = proceeds * self.commission
                cash += (proceeds - commission_cost)
                self.trades.append({"date": date, "action": "sell", "shares": position, "price": price, "commission": commission_cost})
                position = 0
    ...
    def get_trade_log(self) -> pd.DataFrame:
        if self.trades:
            ...
            return pd.DataFrame(columns=["date", "action", "shares", "price", "commission"])
...
def main():
    ...
    position_sizing_method = st.sidebar.radio("Position Sizing Method", options=["Fixed", "Dynamic", "Fixed Fractional", "Risk Parity"])
    if position_sizing_method == "Fixed":
        shares = st.sidebar.number_input("Number of Shares", value=100, step=1)
    else:
        shares = 0
    risk_fraction = 0.01
    if position_sizing_method in ["Fixed Fractional", "Risk Parity"]:
        risk_fraction = st.sidebar.slider("Risk Fraction", min_value=0.01, max_value=0.5, value=0.01, step=0.01)
    commission = st.sidebar.slider("Commission per Trade (%)", min_value=0.0, max_value=1.0, value=0.1, step=0.1) / 100.0
    ...
    config = {
        ...
        "composite_choices": composite_choices
    }
    config_json = json.dumps(config, indent=4)
    st.sidebar.download_button(
        label="Save Configuration",
        data=config_json,
        file_name="quantbacktest_config.json",
        ...
        with st.spinner("Running backtest..."):
            if optimize:
                ...
                if selected_strategy == "SMA Crossover":
                    ...
                    backtester = Backtester(data, signals, initial_capital, shares, position_sizing_method.lower(), risk_fraction, commission)
                    if selected_strategy == "Custom Profit/Stop":
                    ...
                    portfolio = backtester.run_backtest()
            if not optimize:
                trade_log_df = backtester.get_trade_log()
            else:
                trade_log_df = pd.DataFrame()  # No trade log available during optimization
    ...
    if portfolio is not None:
        ...
        with tabs[7]:
            ...
            if not trade_log_df.empty:
                ...
                st.write("No trades recorded during the backtest.")
        with tabs[8]:
